...
class mytrainer(object):
    """Applies an `Optimizer` on a set of Parameters. mytrainer should
 ...
 ###############################################tuned  by chris#########################################
        ...
        if self._kvstore:
#             print(type(self._params_to_init),len(self._params_to_init))
            for param in self._params_to_init:
                if param._deferred_init:
#                     print('here is a deferred init params')
                    ...
                    param_block.append(param_arrays[0])
#                     print('the type of keys',idx, param.name)                 
#                     print('the type of values',type(param_arrays) ,' the length of param_arrays:',len(param_arrays))
...
#             print(isinstance(kvstore, mx.kvstore.KVStore))
    ...
    def step(self, batch_size, ignore_stale_grad=False):
        ...
        nd.waitall()
        time3 = time.time()
#         print('push cost time %.5f, update cost time %.5f'%(time2 - time1,time3 - time2))
#         print('the time of push process began %.5f'%time1)
#         print('the time of push process finished %.5f'%time2)
 ...
 ################################tuned by chris##################################
            ...
            params_list = []
            i_list = []
            for i, param in enumerate(self._params):  #########FIFO#########
...
#     def _oldupdate(self, ignore_stale_grad=False):
        ...
        for i, param in enumerate(self._params):
            ...
            if self._kvstore and self._update_on_kvstore:
                if param._stype == 'default':
                    ...
                    i_list.append(i)
                    if block_num % self._block_num == 0 or block_num == 283:
#                         print('here is pulling layer %d'%i) # 对于每次pull都会经过这里
    ...
    def load_states(self, fname):
        ...
        based on current mytrainer's parameters.
        """
        if not self._kv_initialized:
            self._init_kvstore()
        if self._params_to_init:
